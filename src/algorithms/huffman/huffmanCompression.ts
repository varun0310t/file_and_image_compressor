...
import { BitUtils, BitReader, BitWriter } from "../../utils/bitUtils";

export class HuffmanCompression implements ICompressor {
  private tree: HuffmanTree = new HuffmanTree();
  private originalSize: number = 0;
  private compressedSize: number = 0;
  private compressionLevel: number = 9; // Default maximum compression
  ...
  async compress(data: Buffer): Promise<Buffer> {
    ...
    const symbolTableBuffers: Buffer[] = [];
    ...
    codes.forEach((code, symbol) => {
      ...
      const bitWriter = BitUtils.createBitWriter();
    ...
    console.log(
      "DEBUG: Header size:",
      result.length - compressedDataBuffer.length,
      "bytes"
    );
  ...
  async decompress(data: Buffer): Promise<Buffer> {
    ...
    if (data.length < offset + 2) {
      throw new Error(
        "Invalid compressed file format - truncated symbol count"
      );
    }
    ...
    const codeToSymbol = new Map<string, number>();

    for (let i = 0; i < uniqueSymbols; i++) {
      // Read symbol (1 byte)
      if (offset >= data.length) {
        throw new Error(
          `Invalid compressed file format - truncated at symbol ${i}`
        );
      }
      ...
      if (offset >= data.length) {
        throw new Error(
          `Invalid compressed file format - truncated at code length for symbol ${symbol}`
        );
      }
      ...
      if (offset >= data.length) {
        throw new Error(
          `Invalid compressed file format - truncated at code bytes count for symbol ${symbol}`
        );
      }
      ...
      if (offset + codeBytes > data.length) {
        throw new Error(
          `Invalid compressed file format - truncated at code data for symbol ${symbol}`
        );
      }

      // Extract code bits
      let code = "";
      const bitReader = BitUtils.createBitReader(
        data.slice(offset, offset + codeBytes)
      );
      ...
      try {
        ...
        if (error instanceof Error) {
          throw new Error(
            `Error reading code bits for symbol ${symbol}: ${error.message}`
          );
        } else {
      ...
      console.log(
        `DEBUG: Symbol ${symbol} = Code "${code}" (${codeLength} bits)`
      );
    }
    ...
    if (offset >= data.length) {
      throw new Error(
        "Invalid compressed file format - truncated at padding info"
      );
    }
    ...
    const bitReader = BitUtils.createBitReader(compressedData);
    let currentCode = "";
    ...
    try {
      ...
      while (bitsProcessed < totalBits && resultIndex < originalSize) {
        ...
        if (codeToSymbol.has(currentCode)) {
          ...
          if (resultIndex < originalSize) {
            ...
            throw new Error(
              `Decompression overrun - trying to write beyond buffer size ${originalSize}`
            );
          }
        }
        ...
        if (currentCode.length > 100) {
          throw new Error(
            `Potential decoding error - code length exceeds 100 bits: ${currentCode}`
          );
        }
      }
    } catch (error) {
      if (error instanceof Error) {
        throw new Error(
          `Error during bit reading: ${error.message}\nDecoded ${resultIndex} of ${originalSize} bytes`
        );
      } else {
        throw new Error(
          `Error during bit reading\nDecoded ${resultIndex} of ${originalSize} bytes`
        );
      }
    }
    ...
    if (resultIndex !== originalSize) {
      throw new Error(
        `Decompression incomplete: decoded ${resultIndex} of ${originalSize} bytes`
      );
    }
  ...
  getMetadata(): Record<string, any> {
    return {
      ...
      uniqueSymbols: this.tree.codes?.size || 0,
